...
| `entropy_anomaly` | L2 *(weak, advisory)* | bool | The encryption canary: flags restored **text-class** files (`.sql`, `.php`, `.json`, …) whose bytes are near-random — the shape ransomware leaves behind. Always passes; the ANOMALY flag in the evidence is the whole signal. Media, archives, and unknown extensions are never judged (high entropy is their normal state), one odd file stays quiet (≥2 files and ≥10% must look random), and no malware detection is claimed — this is a signal, not a scanner. |
| `file_count` | L2 | `{glob, min_size, expect}` | The content-shaped assertion: count restored files matching `glob` (each at least `min_size` bytes; omit for any size) and evaluate the count against [`expect`](#the-expect-predicate) — e.g. `{glob: "**/*.jpg", min_size: 1, expect: "> 50"}` for "more than 50 non-empty JPEGs restored". A pattern without `/` matches file names at any depth; with `/` it matches the restore-relative path, `**` spanning directories. With `scope: sample` the count reflects the sample — use `scope: full` or an `include_paths` covering the globbed area. |
...
| `tcp` | L3 | int port | The generic "service answers" probe: `fail` unless something inside the sandbox accepts connections on the port (the sandbox has no external network, so the probe runs inside it via bash's `/dev/tcp` — an image without `bash` reports `error`). `- tcp: 5432` asserts the booted database is actually listening. |
